...
\paragraph{}
\todo[inline]{Ajouter une introduction qui donne envie. Attirer le lecteur. À la fin de l'introduction, faire un plan détaillant grossièrement chaque parties.}
...
\paragraph{}
\todo[inline]{Faire la distinction entre automatisation et machine learning.}
...
Voici celles qui vont être développées dans cet état de l'art:
\todo[inline]{Enlever la liste de l'intro ou bien reformuler.}
\begin{itemize}
\item Les réseaux de neurones.
\item Les arbres de décision.
...
\paragraph{}
\todo[inline]{Ajouter une définition du FOREX}
...
\todo[inline]{Regarder et ajouter le différentiel d'intérêt, i.e le carry}
...
\todo[inline]{Relire et corriger si cela n'a pas de sens.}
...
\paragraph{}
\todo[inline]{Ajouter une explication de pourquoi -> "régression pour trouver une fonction" et du comment -> "les équations permettent de trouver cette fonction"}
...
en utilisant \todo[inline]{Mettre un exemple de distribution logistique.} une distribution logistique cumulative. Cette dernière a une forme semblable à une distribution Gaussienne, 
mais avec des queues épaisses et donc une \textit{kurtosis} plus élevée. La \textit{kurtosis} étant définie comme suit:
\todo[inline]{Définir $\mu_4$}
\begin{center}
...
\todo[inline]{Préciser qu'il faut reconstruire l'arbre à chaque entraînement.}
\begin{itemize}
...
\paragraph{}
\todo[inline]{Ajouter ref pour la première phrase. Ça vient de wiki.}
...
\todo[inline]{Ajouter une phrase qui explique que c'est les rendements positifs sur les rendements négatifs.}
...
\begin{itemize}
 ...
 \item \todo[inline]{Réécrire.}Avec la définition de $\Sigma$, un large total de \textit{trading} défavorable\footnote{Calculé par :
...
\todo[inline]{Reformuler la phrase dessous, l'idée est "on a plusieurs manière d'optimiser, c'est un problème classique", ajouter une transition plus smooth avec la descente du gradient, genre "d'ailleurs afin d'optimiser les réseaux de neurones on peut utiliser la descente du gradient".}
...
\paragraph{}
\todo[inline]{Rephraser pour éviter la confusion avec auteur == Romain ?}
...
\todo[inline]{Dans les tableaux, détailler les algorithmes.}
\paragraph{}
...
\subsubsubsection{Résultats}
\paragraph{}
\todo[inline]{Ajouter de la précision dans la légende des tableaux pour les algos.}
...
\todo[inline]{Ajouter le "à quoi", après "peut être une solution", une solution à quoi ?}
\todo[inline]{Nuancer le propos, et dire que c'est UNE solution possible, mais qu'elle est juste d'un point de vue mathématique.}
...
\todo[inline]{Définir $M$, préciser pourquoi $w_{i,t}$}
$$F_t = sign(\sum_{i=0}^M w_{i,t} r_{t-i} + w_{M+1,t} F_{t-1} + v_t)$$
...
Cette formule mathématique implémente un réseau de neurones. Il s'agît d'une multiplication vectorielle entre les poids ($w$) et les \textit{returns}
($r_i$). Les poids ont pour but de donner une pondération à chacun des éléments afin de fournir un résultat le plus "juste" possible, suivant certains
...
\paragraph{}
\todo[inline]{Inverser définition de $S$ et la dévirée de $S$.}
...
\paragraph{}
\todo[inline]{Montrer la référence qui dit cette hypothèse d'équivalence.}
...
tendre vers l'infini. C'est un phénomène qui peut arriver dans des domaines et des projets différents \todo{Ajouter une ref}. Une solution possible pour pallier ce problème 
consiste à appliquer un facteur $f < 1$ afin d'empêcher les poids de croître trop rapidement et de tendre vers l'infini. \todo{Expliquer en quoi ça aide, et citer l'article}
...
humaine pour trouver les meilleures valeurs possibles. C'est une solution assez courante dans le domaine du \textit{machine learning} \todo{Ajouter une ref.}. Cette méthode est
...
Les paramètres ont initialement une valeur aléatoire $\in [0,1]$ car nous n'avons aucune indication sur leur valeur optimale. \todo[inline]{Réécrire la partie ci-dessous} Concernant la génération
...
\paragraph{}
\todo[inline]{Étoffer un peu cette partie.}
...
neurones \todo{Citer Keras et Tensorflow}, cette divergence existait également. Donc pour des données similaires, avec un algorithme d'optimisation similaire mais un réseau de neurones
...
\end{itemize}
  ...
  \todo[inline]{Reformuler la partie ci-dessous, et ptetr mettre une \~ sur le =.}
  Les nouvelles mesures sont calculées :
 ...
 De plus, nous ne connaissons pas le domaine dans lequel chercher les solutions ce qui handicape grandement la recherche. Si la déviation 
 standard est trop importante cela fait diverger les valeurs et nous obtenons de très mauvais résultats \todo{Détailler le "très mauvais"} et si elle est trop petite, nous restons dans
 ...
 Nous n'avons pas de solution pour cette partie et il conviendrait de chercher une solution dans des recherches futures.
  ...
  \todo[inline]{Avant les résultats ajouter un pseudo-code résumant le tout}
\subsubsection{Résultats}
 \paragraph{}
 \todo[inline]{Refaire tous les graphiques.}
 \todo[inline]{Ajouter le grain des données.}
 \todo[inline]{Comparer les signaux entre eux sur deux runs.}
  \todo[inline]{Prendre les exemples déjà fait, mais décomposer en plusieurs graphiques (par ex diviser en 5, pour bien tout voir)}
...
une optimisation des paramètres toutes les $200$ itérations durant la phase d'apprentissage, $\delta = 0.001, \eta=0.001,\rho=0.01, x = 0.005, y=0.0001$.}
\end{figure}

\paragraph{}
D'après ces deux exemples, les résultats peuvent varier d'une \textit{run} à l'autre. Cela affecte le signal et par extension le profit cumulé. Il a peu
de moment où le signal est nul à cause du \textit{stop trailing loss}. Cela provient du fait que si la valeur de $x$ est trop grande, nous ne dépassons
...
les suppositions faîtes précédemment à savoir que :
\begin{itemize}
...
différences entre deux générations.
 ...
 \paragraph{}
 \todo[inline]{Faire plusieurs boucles d'optimisation, genre 100, et ploter à chaque fin, le sharpe ratio.}
...
\end{figure}
 ...
 \todo[inline]{Avant la conclusion ajouter une discussion des performances et du temps: temps de calcul, phases d'optimisations, temps,...}
\subsection{Conclusion}
\paragraph{}
