...
\paragraph{}
L'algorithme va donc, à partir d'un ensemble de méthodes et d'une observation, calculer les classes. Si une de ces dernières est représentée de manière majoritaire\footnote{\textit{i.e.} 50\% des voix + 1 voix}, le programme retournera ce résultat. Si la majorité n'est pas atteinte, aucune classe n'est jugée valable et donc aucune réponse ne sera rendue.
...
Le choix a donc été fait de partagé l'ensemble de données en deux sous-parties. Une première contenant 80\% des données qui sera dédiée à l'entraînement et une seconde avec 20\% pour l'évaluation.
...
\footnotetext{Source: [\ref{descente du gradient stochastique}]}
...
\stepcounter{footnote}\footnotetext{Source des résultats: [\ref{descente du gradient stochastique}].}
...
\footnotetext{Source des résultats: [\ref{descente du gradient stochastique}]}
...
\item "\textit{Real investors eclipsed by fast trading}" du Financial Times, 2012, \url{https://www.ft.com/content/da5d033c-8e1c-11e1-bf8f-00144feab49a?mhq5j=e1} \label{real investors}.

\item "\textit{A Machine Learning Approach to Automated Trading}" de Ning Lu, 09.05.2016. \label{machine learning automated trading}

\item "\textit{An efficient implementation of the backtesting of trading strategies.}" de Ni, Jiarui, et Chegqi Zhang, 2005.

\item "\textit{Algorithmic Trading: Winning Strategies and Their Rationale ( Wiley Trading Series)}" de John Wiley et Sons, 2013.

\item "\textit{Machine Learning}" de Tom Mitchell, 1997. \label{mitchell}

\item "\textit{Support vector machine}" de Wikipédia, \url{https://fr.wikipedia.org/wiki/Machine_\%C3\%A0_vecteurs_de_support}. \label{wikipedia svm}

\item "\textit{Online Machine Learning Algorithms For Currency Exchange Prediction}" de Eleftherios Soulas et Dennis Shasha. \label{descente du gradient stochastique}

\item  "\textit{Algorithme du gradient}" de Wikipédia, \url{https://fr.wikipedia.org/wiki/Algorithme_du_gradient}.\label{wikipedia descente du gradient}

\item "\textit{Descision Tree Learning}" de Tom M. Mitchell.\label{id3}

\item "\textit{Option}" d'Investopedia, \url{http://www.investopedia.com/terms/o/option.asp}.

\item "\textit{Support Vector Machine (and Statistical Learning Theory) Tutorial}" de Jason Weston, \url{http://www.cs.columbia.edu/~kathy/cs4701/documents/jason_svm_tutorial.pdf}.\label{svm equation}

\item  "\textit{An Introduction to Neural Networks}" de Vincent Cheung et Kevin Cannons, \url{http://www2.econ.iastate.edu/tesfatsi/NeuralNetworks.CheungCannonNotes.pdf}.

\item "\textit{Artificial Neural Networks: A Tutorial}" de Anil K. Jain, Jianchang Mao et K.M. Mohiuddin, \url{http://csc.lsu.edu/~jianhua/nn.pdf}.

\item "\textit{Backtesting}" d'Investopedia, \url{http://www.investopedia.com/terms/b/backtesting.asp}. \label{backtesting investopedia}


\item "\textit{Les Accords de Bretten Woods}" de Wikipédia,\url{https://fr.wikipedia.org/wiki/Accords_de_Bretton_Woods}.\label{wikipedia bretten woods}

\item "\textit{Comment on est passé de l’étalon-or à l’étalon-dollar}" d'Addison Wiggin,\url{http://la-chronique-agora.com/etalon-or-etalon-dollar/}. \label{étalon-or à étalon-dollar}

\item "\textit{CHAPITRE 1: LE MARCHE DES CHANGES Monnaie et Finance Internationales}" de David Guerreiro, \url{https://economix.fr/docs/1045/chap_1_2015-16.pdf}. \label{marche des changes}

\item "\textit{La règle des trois unités du marché des changes}" d'Estelle Mermet, \url{http://www.forex.fr/newslist/8696-la-regle-des-trois-unites-du-marche-des-changes}. \label{site fr forex}

\item "\textit{Bid And Asked}" d'Investopedia, \url{http://www.investopedia.com/terms/b/bid-and-asked.asp}. \label{investopedia bid ask}

\item "\textit{Broker}" de Wikipédia, \url{https://en.wikipedia.org/wiki/Broker}. \label{wikipedia broker}

\item "\textit{Régression Statistique}" de Wikipédia, \url{https://fr.wikipedia.org/wiki/R\%C3\%A9gression_(statistiques)} \label{wikipedia régression}
