...
\title{Apprentissage automatique dans le cadre du \textit{FOREX}}
...
donc de gagner en vitesse tout en gardant, voir en augmentant, la qualité des résultats.
...
\end{itemize}
 ...
 Une amélioration possible serait d'effectuer cette boucle d'optimisation plusieurs fois afin d'améliorer la qualité de \textit{Sharpe raito}, dans la littérature ceci est nommé \textit{epochs}. Malheureusement ce changement n'améliore pas les résultats, dans le meilleur des cas, voir les dégrade si le nombre d'\textit{epochs} est trop important. L'algorithme n'étant pas stable et cela peut influencer au fil des itérations d'optimisation pour finir par avoir un effet néfaste.
...
\begin{sidewaysfigure}
	...
	\caption[Blup]{2000 points du cours EUR/USD entre le 01.01.2004 et le 01.01.2006, le \textit{Sharpe ratio} positif obtenu par l'algorithme durant l'entraînement et le \textit{p\&l} obtenu grâce aux prédictions. Avec comme paramètres $|w| = 20$, $2000$ éléments d'entraînement, $2000$ de test, une optimisation des
...
\begin{sidewaysfigure}
	...
	\caption[Blup]{2000 points du cours EUR/USD entre le 01.01.2004 et le 01.01.2006, le \textit{Sharpe ratio} négatif obtenu par l'algorithme durant l'entraînement et le \textit{p\&l} obtenu grâce aux prédictions. Avec comme paramètres $|w| = 20$, $2000$ éléments d'entraînement, $2000$ de test, une optimisation des
		paramètres toutes les $200$ itérations durant la phase d'apprentissage, $\delta = 0.001, \eta=0.001,\rho=0.01$
		\footnotemark$, x = 0.005, y=0.0001$. }
...
\begin{sidewaysfigure}
	...
	\caption[Blup]{Les mêmes paramètres et valeurs que la figure \ref{exemple avec filtrage} sans que le filtrage par \textit{Sharpe ratio} y soit effectué.}
...
Afin de vérifier que les données récupérées n'étaient pas problématique nous avons effectuer deux tests assez simple :
\begin{itemize}
	\item Compter le nombre d'éléments et le comparer au nombre approximatif attendu. Par exemple, en ayant $430000$ éléments à la minute, cela correspond à environ 297 jours. Sachant qu'il y a, si l'on retire les weekend et jours fériés, environ 295 jours ouvrables\footnote{Sans prendre en compte les subtilités du décalage horaire.}, c'est cohérent avec les valeurs récupérées.
	\item Vérifier qu'il n'y a pas de donnée incohérente en terme de valeur. Par exemple sur le cours EUR/USD, l'intervalle de valeurs est de $[0,3]$ en prenant large, nous avons donc testé afin de nous assurer qu'il n'y avait pas d'élément qui excédait ces bornes.
\end{itemize}
...
\subsubsection{Résultats}
 ...
 \begin{sidewaysfigure}
 	...
 	\caption[Blup]{Cours EUR/USD entre le 01.06.2000 et le 31.09.2001, le signal d'achat prédit par l'algorithme ainsi que le \textit{p\&l} résultant de ses prédictions. Avec pour paramètres $|w| = 20$, $2000$ éléments d'entraînement, $500$ de test, une optimisation des
 		paramètres toutes les $200$ itérations durant la phase d'apprentissage, $\delta = 0.001, \eta=0.001,\rho=0.01$
 		\footnotemark$, x = 0.005, y=0.0001$.}
...
\subsubsubsection{Exploration des paramètres}
\paragraph{}
Les graphiques suivants proviennent d'une volonté d'analyser les résultats lorsque l'on fait varier certains paramètres. Afin de pouvoir effectuer une comparaison, nous allons garder le cours EUR/USD et modifier seulement certaines valeurs de l'algorithme qui pourraient avoir une influence significative sur ce dernier. Certains paramètres sont communs à tous les graphiques, nous allons les énoncer ici afin de ne mentionner dans les légendes que les éléments pertinents. La forme sera toujours la même avec le cours EUR/USD durant une certaine période\footnote{La période sera spécifiée comme axe des $x$ dans les graphiques.}, le signal d'achat prédit par l'algorithme ainsi que le \textit{p\&l} résultant de ses prédictions avec $|w| = 20$, une optimisation des paramètres toutes les 200 itérations durant la phase d'apprentissage, $\delta = 0.001, \eta=0.001,\rho=0.01, x = 0.005$ et $y=0.0001$.
...
\begin{sidewaysfigure}
	...
	\caption{Une période d'entraînement de $1000$ éléments puis $250$ de test. Fréquence des données à la minute.}
...
\begin{sidewaysfigure}
	...
	\caption{Une période d'entraînement de $200$ éléments et $50$ de test. Fréquence des données journalière.}
...
\begin{sidewaysfigure}
	...
	\caption{Une période d'entraînement de $200$ éléments et $50$ de test. Fréquence des données à l'heure.}
...
\paragraph{}
Le code du logiciel est disponible sous Github à cette adresse : \url{https://github.com/mencattini/eMuLATe}. Le choix du langage s'est porté sur Kotlin afin d'avoir une facilité de développement proche d'un langage de \textit{scripting} tout en gardant la compatibilité et les avantages de la JVM. Tout les tests et les \textit{benchmarks} ont été réalisés sur Lenovo ThinkPad T420S avec un Intel Core i5.
...
de tous les paramètres sont extrêmement difficiles à trouver. La moindre petite variation peut aboutir à un crash ou à un profit \texttt{NaN}. Le caractère
...
Pour les méta-paramètres, il y a plusieurs points à soulever. Il faudrait dans un premier temps trouver les domaines des valeurs afin de permettre
...
RMSProp, cela permettrait d'améliorer la vitesse tout en proposant des méthodes d'optimisation plus précises et donc d'augmenter la performance globale.
...
vitesse de convergence, diminuer le nombre de données nécessaire et réduire le taux d'erreur. En sacrifiant du temps de calcul, car un réseau de neurones à plusieurs couches est plus
